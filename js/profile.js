...
let cachedTelegramId = null;

// Функция получения Telegram ID
export async function getTelegramIdAsync() {
  if (cachedTelegramId) return cachedTelegramId;
  ...
  return new Promise((resolve) => {
    const tryGetId = () => {
      ...
      if (telegramId) {
        ...
        return resolve(telegramId);
      }

      if (window.Telegram?.WebApp?.initDataUnsafe?.user?.id) {
        cachedTelegramId = window.Telegram.WebApp.initDataUnsafe.user.id;
        return resolve(cachedTelegramId);
      }

      // Якщо не готово — пробуємо знову через 100мс
      setTimeout(tryGetId, 100);
    };
...
export { getTelegramIdAsync };
// Подключаем профиль при загрузке

getTelegramIdAsync().then((telegramId) => {
  if (telegramId) {
